...
pub fn build_environment(packages: Vec<MovePackage>) -> GlobalEnv {
    ...
    dependencies(&mut packages, &mut modules, &structs, &identifier_map);
    type_and_direct_dependencies(&mut packages, &modules, &package_map);
...
fn type_and_direct_dependencies(
    packages: &mut [Package],
    modules: &[Module],
    package_map: &BTreeMap<ObjectID, PackageIndex>,
) {
    packages.iter_mut().for_each(|package| {
        let mut modules_dep_set: BTreeSet<ObjectID> = BTreeSet::new();
        package.modules.iter().for_each(|module_idx| {
            let module = &modules[*module_idx];
            modules_dep_set.extend(module.dependencies.iter());
        });
        let packages_idxs = modules_dep_set
            .iter()
            .map(|module_dep| {
                *package_map.get(module_dep).unwrap_or_else(|| {
                    panic!("Unable to find package index for package {}", module_dep,)
                })
            })
            .collect::<BTreeSet<_>>();
        packages_idxs.iter().for_each(|package_idx| {
            if *package_idx != package.self_idx {
                package.direct_dependencies.insert(*package_idx);
            }
        });
    });
}
