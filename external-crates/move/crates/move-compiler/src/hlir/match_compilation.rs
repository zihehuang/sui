...
pub fn compile_match(
    ...
    let exp_value = sp(eloc, T::UnannotatedExp_::Block((dummy_use_funs(), seq)));
...
fn make_arm_unpack(
    ...
    let body = T::UnannotatedExp_::Block((dummy_use_funs(), seq));
...
// TODO(cgswords): these are not necessary once macros land, as they are removed during translation
// in typing after that. This is a stop-gab that will also go away then, since we are only
// introducing this right before HLIR translation.
fn dummy_use_funs() -> UseFuns {
    UseFuns {
